...
internal class AwaitPromiseBuilder<T> {
    ...
    func timeout(timeoutInterval: TimeInterval, forcefullyAbortTimeout: TimeInterval) -> Self {
        // = Discussion =
        //
...
internal class Awaiter {
    ...
    func performBlock<T>(
        closure: @escaping ((T) -> Void) throws -> Void) -> AwaitPromiseBuilder<T> {
            let promise = AwaitPromise<T>()
    ...
    func poll<T>(pollInterval: TimeInterval, closure: @escaping () throws -> T?) -> AwaitPromiseBuilder<T> {
        let promise = AwaitPromise<T>()
        let timeoutSource = createTimerSource(timeoutQueue)
